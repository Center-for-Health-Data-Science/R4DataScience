...
8.  Read in the file from `data/diabetes.csv` using the `read_csv()`. In order to use `read_csv()` you must first load the R-package `tidyverse`, as the function comes from this package. 
Check the structure of the data with the `str()` function. 
**Re-save the file!**
