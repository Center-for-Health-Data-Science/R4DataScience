...
1.  To get this material you went to the github repository of this course: https://github.com/Center-for-Health-Data-Science/R4DataScience, pressed **Code** → **Download ZIP** and downloaded the directory with the course material called *R4DataScience* or *R4DataScience-main*. Move the course materials folder to a place on you computer you can easily find!

This repository you just downloaded contains several sub directory with more and less relevant material. The files you need for this course is:
...
2.  Under the course directory, make a sub directory for the outputs that you will generate doing the exercises, name it `output`.
...
**N.B** IF you are working on a windows computer and you copy a path from your file explorer to paste into Rstudio, you will have to change the direction of all the slashes in the path (backward to forward).

## R Script

4.  Create an R script and save it in your `output` folder.

An R script is a plain text file containing a series of R commands and code used for data analysis. R scripts have a `.R` extension and can be executed line-by-line in an interactive R session or as a whole script. They are ideal for automating workflows and keeping your analyses reproducible and organized. R scripts can be submitted to a job on a super computer unlike Quarto documents.
...
*HINT:* if you are unsure what path you want to use go to session --\> set working directory --\> choose directory and select the folder you want to be your working directory. Then, copy the path from the console and paste it into the `setwd()` function.

7.  Run `getwd()` again.

8.  Read in the file from `data/diabetes.csv` using the `read_csv()`. In order to use `read_csv()` you must first load the R-package `tidyverse`, as the function comes from this package. Check the structure of the data with the `str()` function. **Re-save the file!**
...
9.  Create an Quarto document and save it in your `output` folder.
...
14. Close all open script/tabs in your session and close Rstudio down. Re-open Rstudio. Now, create an R project by clicking the *Project* in the top right → *New Project* → *Existing Directory* and choose the _course materials folder_. Look at the top-right corner to check that you are in your R project.

15. Reopen the the R script and Quarto document you created in Exercise 4 and 9 respectively. Check each of their working directories. Are they as you expect? Explain.
