...
Let's try to look just at couple of our plots. Only collagen genes that start with 1 "COL1..", `tumorstage` and `age_at_initial_path_diagn.`
...
corr_matrix <- df_comb %>% select(where(is.integer)) %>% cor(use = "complete.obs")
...
# All Integer Variables
corr_matrix <- df_comb %>% select(starts_with("COL")) %>% cor(use = "complete.obs")
...
Visualization is essential for understanding relationships — but it has limits. As dimensionality grows, traditional plots get harder to interpret. It is time to transition to PCA or clustering to uncover deeper structure.
