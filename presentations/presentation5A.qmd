...
In this case the p-value is significant which means that the more complex model, that includes exercises, is a better fit. Discuss in small groups if you think this makes sense with the exercises levels not being significant predictors when we looked at `summary(model_log)`.
