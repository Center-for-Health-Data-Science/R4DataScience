...
## Linear Regression
TODO: this model - these variables?
...
train <- df_smoke %>% sample_frac(0.75)
nrow(train)
head(train)
...
```{r}
# Predict life expectancy for each combination
...
TODO: Below where does excise come in? should it be $$life=b_0+b_1*cigarettes+b2_exercise$$ ?
...
TODO: below, should it be: Our $y$ is either 0 (no, the person is not dead before 75) or 1 (yes, the person is dead before 75)
...
And if we want to know what this means for the probability of dying before 75 we just take the logit of $y$ :

$$ P(earlydeath == 1) = \frac{1}{1+ e^{(-y)}} $$

Which makes the link between what we're actually interested in (people's chances of dying before 75) and what we're modelling the logit. End of math.
...
### Model interpretation
TODO: two times "high amount of exercise". Where should the low amount of exercise go?
We see from looking at the summary that the coefficient of exercise level 1 and level 2 is not significant. This means that we are not confident that doing a high amount of exercise has a significant impact on the probability of dying before 75 compared to doing moderate or high amounts of exercise. This does not mean that there can be no influence, merely that we do not have enough data to detect it if it is there.
...
TODO: should we go forward with the model_reduced since that performs better?
