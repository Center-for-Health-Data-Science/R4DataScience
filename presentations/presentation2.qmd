...
```{r warning=FALSE}
df_comb %>%
  dplyr::filter(Sample_code  == "Tumor")  %>%
...
ggpairs(na.omit(df_comb %>% 
          select_if(is.factor)), columns = 3:6,  ggplot2::aes(colour = batch)) + 
  theme_bw()  + 
...
Understanding which category your data falls into helps guide how to deal with it. Let’s inspect missing values in the `tumorstage` variable:
...
```{r}
df_comb <- df_comb %>% filter(Sample_code == "Tumor")
```
...
```{r}
save(df_comb, file="/Users/pbj825/Desktop/R4DataScience/data/Ovarian_comb_prep.RData")
```
