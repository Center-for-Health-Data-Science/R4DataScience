...
Instead of visualizing variables one by one, we can also look at samples across a set of variables and identify outliers that way. If we have a lot of integer and numeric values in our dataset, one thing that might make sense is to make a dendogram (tree) using hierarchical clustering and look at if any sample 'stand out'.

Lets try this for the integer variables in our dataset. 
...
From our plot we see that a couple of samples seem to have very short branch lengths (which may indicate and outlier). One such sample is 208 (row id). Lets have a look at it:
...
df_int[208, ]
```
We can now compare the values of this sample to the distributions above to try an figure out what makes it an outlier and IF it should be removed. 
