---
title: "Presentation 2: Summary Statistics"
format: html
project:
 type: website
 output-dir: ../docs
...
Now that we've cleaned our data, it’s time to dig a bit deeper into the actual contents of our dataset. Exploratory Data Analysis (EDA) is an essential step in any data analysis or modeling workflow.
...
Now, we load our dataset which was pre-processed and saved as an `.RData` file in the end of presentation 1:
...
Depending on how you want to investigate the variables, there are different tools in your EDA toolkit:
...
The creed of `ggplot` is that every information you want to plot must be in a column. There is one column that describes the x-axis, one for the y-axis, and one for each additional aesthetic like color, size, shape, ect.

Your data might not be in the correct format for all the plots you want to make. Let’s say we want to create a single plot and compare several **CXCL gene expression levels** across different categories.
...
```{r, warning=FALSE}
df_comb %>% 
  filter(!(is.na(tumorstage) | is.na(vital_status))) %>% 
  ggplot(aes(x = tumorstage, 
             fill = vital_status)) +
    geom_bar() +
    scale_fill_viridis_d() + 
    theme_minimal()
...
We will now have a look at the distribution of levels of the categorical variables across `vital_status`. 
```{r}
...
stats <- df_comb %>%
  ...
  pivot_wider(names_from = statistic, 
              values_from = value) %>% 
  mutate(threshold_upper = mean + 3 * sd,
         threshold_lower = mean - 3 * sd) %>%
...
ggplot(df_comb, 
       aes(x = percent_tumor_cells, y = percent_not_cancer_cells, color = percent_stromal_cells)) +
  geom_point()  +  
  geom_point(data = df_comb %>% filter(is_outlier == TRUE),
