...
```{r warning=FALSE}
df_comb %>%
  filter(percent_tumor_cells == 0 & percent_not_cancer_cells == 0)

df_comb <- df_comb %>%
  filter(
    (percent_tumor_cells != 0 | is.na(percent_tumor_cells) & 
            percent_not_cancer_cells != 0) | is.na(percent_not_cancer_cells))
```
...
```{r}
# Replace NA in x with the median of x (excluding NAs)
