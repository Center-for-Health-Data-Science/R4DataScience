...
# df_ov$age_at_initial_pathologic_diagnosis %>% 
#   hist()
```
...
`~ mean(is.na(.)) < threshold` explained:
`~` = “Define a function”
`.` is a placeholder for the input (in this case, a column of the data frame)
`mean(is.na(.))` calculates the proportion of missing values
`< threshold` returns TRUE or FALSE — which where() uses to decide if a column should be kept
...
01.{0,1} means 01 + . + 0 or 1 of any character (A, B, C, X)
```{r}
df_ov <- df_ov %>% 
