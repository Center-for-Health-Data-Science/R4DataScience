...
Firstly, lets take a look at the variable `vital_status` from the clinical metadata. This variable contains information about whether the patient is alive or deceased and in the analysis below we will denote this our outcome / response variable.
```{r}
# output: vector, base R
df_ov$vital_status %>% 
  head()


# Goutput: vector the tidyverse
df_ov %>%
  select(vital_status)
...
Example of irrelevant variable: `histological_type` has 99% of the same value, so it’s not useful for analysis. We can drop it with `select()` and a minus sign.
```{r, warning=FALSE}
df_ov <- df_ov %>% 
...
```{r results="show"}
df_ov <- df_ov %>% 
  mutate(dominant_cell_type = if_else(percent_not_cancer_cells > percent_tumor_cells, "not_cancer_cells", "cancer_cells"))
...
```{r}
str_detect(df_ov$sample_code, '01') %>% 
  table(useNA = 'ifany')
```

```{r}
str_detect(df_ov$sample_code, '11') %>% 
  table(useNA = 'ifany')
```
...
df_ov %>% 
  select(sample_code, sample_code_manual) %>% 
  slice(100:110)
```
...
df_ov %>% select(alt_sample_name, 
                 sample_code, 
                 sample_code_manual, 
                 sample_recode) %>% tail()

df_ov <- df_ov %>% 
  select(!c(sample_code_manual, 
            sample_code, 
            alt_sample_name, 
            sample_type))
...
df_ov_subset <- df_ov %>% 
  arrange(unique_patient_ID) %>% 
  slice(3:7) %>% select(1:3, vital_status)
