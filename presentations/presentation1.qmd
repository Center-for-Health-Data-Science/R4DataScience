...
Let’s start by taking a look at our dataset to get a feel for the data distribution and identify any potential data-related issues.
...
df_ov %>% 
  ...
  # barplot(las=2, cex.names=0.6, col = "#826AED") # baseR barplot since we are plotting a vector
```
...
Let's have a closer look at the unique values of this variable. Run the code in the console for a more detailed look.
...
To understand the spread of a variables like `age_at_initial_pathologic_diagnosis`, we can check the minimum and maximum values. This helps us to spot outliers or strange values (like someone who’s 200 years old!).

```{r}
df_ov %>% 
  select(age_at_initial_path_diagn) %>% 
  pull() %>% 
  hist()

# df_ov$age_at_initial_path_diagn %>% 
#   hist()
```

Let’s say we want to sort the data according to `age_at_initial_pathologic_diagnosis`:

```{r}
df_ov <- df_ov %>% 
  arrange(desc(age_at_initial_path_diagn)) 
...
1.  If a variable has more than \~10/30% missing data, it probably won’t be useful. Let’s remove those:
...
```{r, warning=FALSE}
target <- c("histological_type", "primarysite")
...
```{r}
# Vital status
df_ov$vital_status %>% table()

df_ov <- df_ov %>% 
  mutate(vital_status = case_when(vital_status == "NA" ~ NA_character_, TRUE ~ vital_status) )
...
df_ov$percent_not_cancer_cells %>% hist(col = 'lightblue')
```
...
```{r}
str_detect(df_ov$Sample_code, '01') %>% head()
```

```{r}
str_detect(df_ov$Sample_code, '11') %>% head()
```

Add a column that records TRUE if sample is a primary tumor and places it after the `Sample_type` column. 01.{0,1} means 01 + . + 0 or 1 of any character (A, B, C, X)

```{r}
df_ov <- df_ov %>% 
  mutate(Sample_code_manual = str_detect(df_ov$Sample_code, '01.{0,1}'), 
         .after = sample_type)
...
df_ov <- df_ov %>% 
  select(-Sample_code_manual, -Sample_code, -alt_sample_name, -sample_type)
