...
Or we can choose to execute our function once for every element of an iterable, e.g. every row in a df:

```{r}
df <- tibble(age = c(45, 16, 31, 56, 19),
...
```{r}
my_boxplot <- function(df, var = ''){
  
  p <- ggplot(data = df, 
         aes(y = .data[[var]])) + # Use variable as column reference
  geom_boxplot(fill='steelblue') + 
  theme_minimal() + 
  labs(title = paste('Boxplot of', var)) # Use variable as string
...
```{r}
my_boxplot(df = df, 'age')
```

Or equivalently

```{r}
my_boxplot(df = df, colnames(df)[1])
```
