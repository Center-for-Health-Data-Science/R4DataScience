...
5.  As you will use the response `Outcome.Birth`, you should remove the other four outcome measures from your dataset.
...
```{r}
optML <- optML %>% 
  dplyr::select(-c(Preg.ended...37.wk, Apgar1, Apgar5, GA.at.outcome, Birthweight)) %>%
...
```{r}
train <- train %>% 
  dplyr::select(-c(PID, Outcome.Birth))
