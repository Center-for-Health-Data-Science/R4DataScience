...
#make the plot
ggplot(diabetes_glucose, aes(y = !!sym(plot_column))) +
  geom_boxplot()
...
3. Add a check to your function whether the supplied column is numeric. Note here that you need to test the data type of the column you want to plot, not the data type of it's name. Confirm that your check works.
...
4. Write code to apply your boxplot function to each numerical column in the dataframe. There are different ways to achieve this.
...
glucose_group_mean <- diabetes_glucose_unnest %>%
  group_by(Measurement, !!sym(category)) %>%
