...
-   Both `Married` and `Work` has a lot of mistakes. From looking at the mistakes the mistakes can be fixed like this: `Married` is changed to 'No' if an 'N' is present in the string and 'Yes' if 'Y' is present. For `Work` the values that contain a capital "S" can be changed to "Self-employed", "Retired" it looking fine, and for "Public" and "Private", we need to correct these manually.
